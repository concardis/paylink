...
$invoice->setDescription('Thanks for using Paylink to pay your order');

// administrative information, which provider to use (psp)
// psp #1 = Paylink' test mode, see http://developers.pay-link.eu/docs/miscellaneous
$invoice->setPsp(1);
...
try {
    $response = $paylink->create($invoice);
    var_dump($response);
